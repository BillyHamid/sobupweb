  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-end justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-black text-gray-900">Mes Groupes de Travail Thématiques</h1>
          <p className="text-sm text-gray-500 mt-1">Suivez les actualités, échangez avec vos pairs et accédez aux ressources des GTT auxquels vous appartenez.</p>
        </div>
        <Link href="/gtt"
          ...
          <Activity className="w-4 h-4" /> Explorer tous les GTT
        </Link>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { value: String(userMemberships.length), label: "GTT actifs", icon: Users, color: "#31B9AE" },
          { value: String(userMemberships.length * 4), label: "Discussions en cours", icon: MessageSquare, color: "#e67e22" },
          { value: String(userMemberships.length * 7), label: "Documents accessibles", icon: FileText, color: "#7c3aed" },
        ].map((s) => {
          const Icon = s.icon;
          return (
      ...
      {userMemberships.length > 0 ? (
        <section>
          <h2 className="font-black text-gray-900 mb-4">Mes GTT ({userMemberships.length})</h2>
                  ...
                  <div className="grid lg:grid-cols-4 gap-0">
                    <div className="lg:col-span-2 p-6">
                      ...
                      <div className="text-xs text-gray-400 space-y-1 mt-3">
                        {info?.coordinateur && (
                          <p>👨‍⚕️ Coordinateur : <strong className="text-gray-700">{info.coordinateur}</strong></p>
                        )}
                        <p>📅 Adhéré le <strong className="text-gray-700">{joinedDate}</strong></p>
                        {m.fonction && (
                          <p>🏥 Profil : <strong className="text-gray-700">{m.fonction}{m.lieuExercice ? ` · ${m.lieuExercice}` : ""}</strong></p>
                        )}
                      </div>
                    </div>

                    <div className="lg:col-span-2 p-6 border-l border-gray-100" style={{ background: "#fafbfc" }}>
        ...
        // État vide
        <div className="bg-white rounded-3xl border-2 border-dashed border-gray-200 p-10 lg:p-14 text-center">
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4"
            style={{ background: "#E8F9F7" }}>
            <Users className="w-8 h-8" style={{ color: "#31B9AE" }} />
          </div>
          <h3 className="font-black text-gray-900 text-lg mb-2">Vous n&apos;avez rejoint aucun GTT</h3>
          ...
          <Link href="/gtt"
            className="inline-flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-black text-white hover:-translate-y-0.5 transition-all hover:shadow-lg"
            style={{ background: "#31B9AE" }}>
      ...
      {disponibles.length > 0 && (
        <section>
          <div className="flex items-end justify-between mb-4">
            ...
            <Link href="/gtt" className="text-xs font-bold flex items-center gap-1" style={{ color: "#31B9AE" }}>
              Voir tous les GTT <ArrowUpRight className="w-3 h-3" />
            </Link>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {disponibles.map((gtt) => (
              <Link key={gtt.name} href={gtt.href}
                className="bg-white rounded-2xl p-5 border border-gray-100 hover:shadow-md hover:-translate-y-0.5 transition-all block">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center mb-3"
                  style={{ background: `${gtt.color}15` }}>
                  <Activity className="w-5 h-5" style={{ color: gtt.color }} />
                </div>
                <p className="font-black text-gray-900 text-sm leading-snug mb-1">{gtt.name}</p>
                <p className="text-xs text-gray-400 mb-3">{gtt.membres} membres actifs</p>
