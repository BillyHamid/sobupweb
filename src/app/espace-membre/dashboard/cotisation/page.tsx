...
export default function CotisationPage() {
  return (
    <div className="space-y-8 max-w-5xl">
      ...
      {/* En-tête */}
      <div>
        <h1 className="text-2xl font-black text-gray-900">Ma cotisation</h1>
        <p className="text-sm text-gray-500 mt-1">Gérez votre cotisation annuelle et consultez l&apos;historique de vos paiements.</p>
      </div>

      {/* Carte statut */}
      <div className="rounded-3xl overflow-hidden p-8 text-white relative"
        ...
        <div className="relative grid lg:grid-cols-3 gap-8 items-center">
          <div className="lg:col-span-2">
            ...
            <h2 className="text-3xl font-black mb-2">Vous êtes à jour ✓</h2>
            ...
            <div className="flex flex-wrap gap-3">
              <button className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-black text-white hover:-translate-y-0.5 transition-all"
                style={{ background: "#e67e22" }}>
                <Receipt className="w-4 h-4" /> Télécharger le reçu
              </button>
              <button className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-black border-2 hover:-translate-y-0.5 transition-all"
          ...
          <div className="rounded-2xl p-5 backdrop-blur-sm"
            ...
            <div className="space-y-2 text-xs">
              {[
                ...
                { l: "Date du paiement", v: "12 janvier 2026" },
                ...
                <div key={d.l} className="flex justify-between">
                  <span className="text-white/65">{d.l}</span>
                  <span className="font-bold text-white">{d.v}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* Avantages */}
      <section>
        <h2 className="font-black text-gray-900 mb-4">Vos avantages en tant que membre à jour</h2>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            <div key={a.titre} className="bg-white rounded-2xl p-5 border border-gray-100 hover:shadow-md transition-shadow">
              <div className="text-2xl mb-3">{a.icon}</div>
              <p className="font-black text-gray-900 text-sm">{a.titre}</p>
      ...
      {/* Historique */}
      <section>
        <h2 className="font-black text-gray-900 mb-4">Historique de mes cotisations</h2>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                ...
                <th className="text-left px-6 py-3 text-xs font-black uppercase tracking-wider text-gray-500 hidden sm:table-cell">Date de paiement</th>
            ...
            <tbody>
              {historique.map((h, i) => (
                <tr key={i} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-xs text-gray-500 hidden sm:table-cell flex items-center gap-1.5">
                    <Calendar className="w-3 h-3" /> {h.date}
                  </td>
                  <td className="px-6 py-4 font-bold text-gray-900 text-sm">{h.montant}</td>
                  <td className="px-6 py-4 text-xs text-gray-500 hidden md:table-cell flex items-center gap-1.5">
      ...
      {/* Sécurité */}
      <div className="rounded-2xl p-5 flex items-center gap-4 border border-gray-100" style={{ background: "#f0f9ff" }}>
        <div className="w-12 h-12 rounded-full flex items-center justify-center shrink-0" style={{ background: "#0ea5e9" }}>
