...
export default function EventsListClient({ events }: { events: PublicEvent[] }) {
  ...
  useEffect(() => {
    if (!openEvent) return;
    const onKey = (e: KeyboardEvent) => { if (e.key === "Escape") setOpenEvent(null); };
  ...
  return (
    <>
        ...
        <div className="mx-auto max-w-7xl px-4 space-y-5">
          {events.length === 0 ? (
            <div className="bg-background rounded-2xl border border-gray-100 p-12 text-center">
              <p className="text-gray-500">Aucun événement publié pour l&apos;instant.</p>
            </div>
          ) : events.map((ev) => {
            const inner = (
              <>
                ...
                <div className="flex-1 p-6 flex flex-col justify-between text-left">
                  <div>
                    <div className="flex items-start justify-between gap-4 mb-3">
                      <h3 className="font-black text-gray-900 text-lg group-hover:text-primary transition-colors leading-tight">{ev.title}</h3>
                    ...
                    {ev.excerpt && <p className="text-gray-500 text-sm leading-relaxed mb-4">{ev.excerpt}</p>}
                  </div>
                  <div className="flex flex-wrap items-center gap-4 text-sm">
                    <span className="flex items-center gap-1.5 text-gray-500">
                      <Calendar className="w-4 h-4" />{ev.display_date}{ev.time_range ? ` · ${ev.time_range}` : ""}
                    </span>
                    <span className="flex items-center gap-1.5 text-gray-500">
            ...
            const className = "group bg-background rounded-2xl overflow-hidden border border-gray-100 flex flex-col md:flex-row hover:shadow-xl transition-all hover:-translate-y-0.5 card-shadow w-full";
            return ev.has_page ? (
              <Link key={ev.id} href={`/evenements/${ev.slug}`} className={className}>{inner}</Link>
            ) : (
              <button key={ev.id} type="button" onClick={() => setOpenEvent(ev)} className={className}
                aria-label={`Ouvrir les détails — ${ev.title}`}>{inner}</button>
            );
          })}
        </div>
      </section>
      ...
      {openEvent && (
        <div role="dialog" aria-modal="true" aria-labelledby="ev-modal-title"
            ...
            <div className="relative h-56 sm:h-64 w-full overflow-hidden shrink-0 bg-gray-100">
              ...
              <div className="absolute bottom-4 left-4 right-4">
                <span className="inline-block text-[11px] font-bold px-2.5 py-1 rounded-full mb-2"
                  style={{ background: openEvent.badge_bg, color: openEvent.badge_color }}>
                  {openEvent.badge_label}
                </span>
                <h2 id="ev-modal-title" className="text-white text-xl sm:text-2xl font-black leading-tight">{openEvent.title}</h2>
              </div>
            </div>
            <div className="p-6 sm:p-8 overflow-y-auto">
              <div className="grid sm:grid-cols-2 gap-4 mb-6">
                <div className="rounded-xl p-4 border border-gray-100" style={{ background: "#f8fafc" }}>
                  <p className="text-[11px] font-black uppercase tracking-widest text-gray-400 mb-1.5 flex items-center gap-1.5">
                    <Calendar className="w-3 h-3" /> Date{openEvent.time_range ? " · Horaire" : ""}
                  </p>
                  <p className="text-sm font-bold text-gray-900">{openEvent.display_date}</p>
                  {openEvent.time_range && <p className="text-xs text-gray-500 mt-0.5">{openEvent.time_range}</p>}
                </div>
                <div className="rounded-xl p-4 border border-gray-100" style={{ background: "#f8fafc" }}>
                  <p className="text-[11px] font-black uppercase tracking-widest text-gray-400 mb-1.5 flex items-center gap-1.5">
              ...
              {openEvent.gtt && (
                <div className="rounded-xl p-4 border mb-6 flex items-center gap-3"
                  ...
                  <div>
                    <p className="text-[11px] font-black uppercase tracking-widest mb-0.5" style={{ color: "#31B9AE" }}>Organisé par</p>
